@if (isset($hasChildren) and !$hasChildren)
	...
	@if (isset($category) and !empty($category))
		...
		@if (isset($category->children) and $category->children->count() > 0)
			<a href="#browseCategories" data-toggle="modal" class="cat-link" data-id="{{ $category->tid }}">
				{{ $category->name }}
			</a>
		@else
			{{ $category->name }}&nbsp;
			[ <a href="#browseCategories"
				 data-toggle="modal"
				 class="cat-link"
				 data-id="{{ (isset($category->parent) and !empty($category->parent)) ? $category->parent->tid : 0 }}"
					...
					@if (isset($categories) and $categories->count() > 0)
						@foreach($categories as $key => $cat)
							<div class="col-lg-2 col-md-3 col-sm-4 col-xs-6 f-category">
								<a href="#" class="cat-link" data-id="{{ $cat->tid }}">
											...
											@foreach ($cols as $iCat)
												...
												<div class="cat-list">
													<h3 class="cat-title rounded">
														...
														@endif
														<a href="#" class="cat-link" data-id="{{ $iCat->tid }}">
															{{ $iCat->name }}
														</a>
													...
													<ul class="cat-collapse collapse show cat-id-{{ $iCat->id . $randomId }} long-list-home">
														@if (isset($subCategories) and $subCategories->has($iCat->tid))
															@foreach ($subCategories->get($iCat->tid) as $iSubCat)
																<li>
																	<a href="#" class="cat-link" data-id="{{ $iSubCat->tid }}">
																		{{ $iSubCat->name }}
																	</a>
																</li>
															@endforeach
