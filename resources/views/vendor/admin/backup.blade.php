		...
		<div class="col-12">
			...
			<div class="card rounded">
				<div class="card-body">
					...
					<p class="card-text">{!! trans('admin.help_backup') !!}</p>
				</div>
			</div>
					...
					<table class="table table-hover table-condensed">
						...
						@foreach ($backups as $k => $b)
							<tr>
								...
								<td class="text-right">
									@if ($b['download'])
										<a class="btn btn-xs btn-secondary" href="{{ admin_url('backups/download/') }}?disk={{ $b['disk'] }}&path={{ urlencode($b['file_path']) }}&file_name={{ urlencode($b['file_name']) }}"><i class="fa fa-cloud-download"></i> {{ trans('admin.download') }}</a>
									@endif
									<a class="btn btn-xs btn-danger" data-button-type="delete" href="{{ admin_url('backups/delete/'.$b['file_name']) }}?disk={{ $b['disk'] }}"><i class="fa fa-trash-o"></i> {{ trans('admin.delete') }}</a>
								</td>
							</tr>
						@endforeach
						</tbody>
